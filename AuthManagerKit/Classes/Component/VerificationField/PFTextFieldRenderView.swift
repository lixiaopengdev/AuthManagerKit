...
class PFTextFieldRenderView: UIView {
    ...
    private func drawData(in cox: CGContext?) {
        if text != nil {
            ...
            if secureTextEntry {
                //画点
                let radius = 3.5  //Double.maximum(minumDotRadius, Double(font.pointSize / 2.0))
                cox!.setFillColor(dotColor.cgColor)
                for index in 0..<text!.count {
                    if index == 3 {
                        ddd = -8
                    }
                    let circlePath = UIBezierPath(arcCenter: CGPoint(x: calculateInputArea(at: index).midX - CGFloat(ddd), y: calculateInputArea(at: index).midY - contentOffSet.y), radius: CGFloat(radius), startAngle: 0, endAngle: CGFloat(Double.pi * 2.0), clockwise: true)
    ...
    private func setup() {
        ...
        showCursor = false
        cursorColor = UIColor.clear
    }
    
    
    private func calculateInputArea(at index: Int) -> CGRect {
        let segementWidth = CGFloat(98) / CGFloat(numOfInput)
//        bounds.size.width / CGFloat(numOfInput)
        var ddd = bounds.size.width/2;
        ddd = (ddd - segementWidth*3 )
        return CGRect(x: CGFloat(index) * segementWidth + ddd, y: 0, width: segementWidth, height: bounds.size.height)
    }
