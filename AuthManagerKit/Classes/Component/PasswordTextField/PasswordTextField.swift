...
open class PasswordTextField: UITextField {
    ...
    func setup()
    {
        ...
        let leftView = UIView(frame: CGRectMake(0, 0, 20, self.bounds.size.height))
        self.secureTextButton.snp.makeConstraints { make in
        ...
        self.rightView = UIView(frame: CGRectMake(0, 0, 44, self.bounds.size.height))
    ...
    open func errorDetailMessages() -> Dictionary<String ,Any> {
        var dict = Dictionary<String, Any>()
        if let text = self.text {
            dict = validationRule.errorMessageDetail(text)
        }
        return dict
    }
